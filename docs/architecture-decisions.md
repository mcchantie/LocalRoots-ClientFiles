...
A generated client-facing document records what the client received, but structured data is required for analytics, filtering, pricing evaluation, and future automation. The current generated format is UTF-8 plain text as defined by ADR-050.
...
The structured estimate record will be the source of truth. The generated estimate document will be an immutable client-facing snapshot. The current generated format is UTF-8 plain text as defined by ADR-050.
...
- Analytics should query structured fields rather than parse generated text files.
...
**Status:** Superseded July 16, 2026 by ADR-050
...
## ADR-049: Client-Specific Uploads and Subset-Based Batch Assignment

**Status:** Accepted July 16, 2026

Client Files will support uploading from a contact-detail page and assigning different subsets of one upload batch to different contacts and categories.

### Client-Detail Upload Behavior

- A contact-detail page includes an **Upload Files** action near the contact's file list.
- Opening the upload flow from a contact page preselects that contact.
- The current contact is clearly displayed in the upload UI.
- The user may explicitly change the assignment or leave selected files unassigned.

### Multi-File Batch Behavior

- Every queued file has a selectable checkbox.
- The user may select all files, individual files, or a range of files.
- Bulk actions apply the selected contact or category only to the currently selected subset.
- Different subsets in the same upload batch may be assigned to different contacts and categories.
- Per-file values may override batch defaults.
- Changing a default does not overwrite a file that already has an explicit override.
- Unassigned files are allowed, but every file must have a valid attachment category before upload begins.

### Rationale

A single upload batch may contain files for several clients or several purposes. Requiring the user to upload each group separately would slow normal office workflows and encourage misclassification.

---

## ADR-050: Estimater Saves UTF-8 Plain-Text Snapshots

**Status:** Accepted July 16, 2026; supersedes ADR-047

Estimater will save each estimate in two connected forms:

1. Structured estimate data in PostgreSQL as the editable source of truth.
2. A versioned UTF-8 plain-text `.txt` file in S3 with a linked Client Files attachment record.

PDF, Google Docs, and Microsoft Word files are not the canonical estimate artifact for the initial integration.

### Client Files Attachment Values

The generated estimate artifact normally uses:

- `content_type = text/plain; charset=UTF-8`
- `.txt` filename extension
- `category = ESTIMATES`
- `file_kind = DOCUMENT`
- `source_system = ESTIMATOR`
- The selected `contact_id`, or `null` for Unassigned
- The related estimate UUID
- `status = READY` after successful storage and verification

### Text-Preservation Requirements

The generated bytes and the viewing workflow must preserve:

- Bullet characters such as `•`
- Em dashes such as `—`
- Tildes such as `~`
- Curly punctuation and accented names
- Spaces, indentation, and line breaks

The text is encoded and decoded as UTF-8. It must not be interpreted as Latin-1, decoded through Base64, converted to HTML, or reparsed as Markdown.

### Revision Rule

An estimate revision creates a new structured version and a new `.txt` snapshot rather than overwriting a file that may already have been copied or sent to a client.

---

## ADR-051: Plain-Text Estimates Use an Authenticated Literal-Text Viewer

**Status:** Accepted July 16, 2026

Client Files will provide an authenticated text-content endpoint for viewing UTF-8 plain-text attachments inside the application.
...
- Verifies the authenticated tenant can access the attachment.
- Returns `text/plain; charset=UTF-8`.
- Removes an optional UTF-8 byte-order mark before returning the visible content.
- Rejects invalid UTF-8 instead of displaying replacement or mojibake characters.
- Applies a configurable maximum plain-text file size.

### Frontend Rendering Rules

- Decode the response as UTF-8.
- Render the value as literal text in a `<pre>`-style component.
- Use `white-space: pre-wrap` so spaces and line breaks remain visible.
- Do not use `innerHTML`.
- Do not run the estimate through Markdown or HTML conversion.
- Copy with `navigator.clipboard.writeText()` so the copied text matches the displayed text.

### Rationale

The saved estimate is intended to be copied into Quo while preserving readable formatting. A literal UTF-8 viewer prevents malformed sequences such as `â€¢` and `â€”`.

---

## ADR-052: File Browsing Supports Grid, List, Search, Sort, and Filters

**Status:** Accepted July 16, 2026

Client Files will provide conventional file-browser controls across All Files, Unassigned, Deleted, and contact-detail file views.

### Required Views

- Grid/icon view with image thumbnails or file-type icons
- List view with sortable metadata columns
- The selected view is persisted for the user, initially through browser-local state

### Search and Sort

The backend supports tenant-scoped, paginated file browsing with:

- Search by display name and original filename
- Category and status filters
- File-kind or content-type filtering where useful
- Sorting by name
- Sorting by creation or upload time
- Sorting by updated time and file size where exposed

Search, sorting, filtering, and pagination must occur server-side for scalable file collections. The frontend may reflect the current state in URL query parameters so browser navigation remains predictable.

### Preview Rules

- Image files display a local thumbnail in the upload queue before upload.
- Stored images display authorized thumbnails or short-lived view URLs.
- Non-image files display an appropriate file-type icon.

---

## ADR-053: Estimater Can Save to an Existing Contact, New Contact, or Unassigned

**Status:** Accepted July 16, 2026

The Estimater customer-messaging area will include a **Save to Client Files** action after an estimate has been generated.

The action is placed below the generated-estimate text area and above the existing **Copy for Text** and **Reset** controls.

### Save Destinations

The user may save the estimate to:

1. An existing contact found by name, phone number, or email address
2. A new contact created during the save flow
3. Unassigned for later review

### New-Contact Rules

- First and last names remain optional.
- A usable phone number or email address is required.
- The new contact is created before the Client Files attachment is initialized.
- The returned contact UUID is used for the estimate attachment.

### Integration Boundary

Estimater generates and stores the structured estimate record, creates the UTF-8 `.txt` artifact, and uses the Client Files attachment workflow for S3 storage and file metadata. The Client Files backend remains responsible for tenant-scoped attachment authorization and storage rules.

---

## ADR-054: Estimate Records Store Front, Back, and Total Lawn Measurements

**Status:** Accepted July 16, 2026

The Estimater `estimates` table will store separate lawn-area measurements when available:
...
- New estimates populate front and back measurements from the calculator request when supplied.
- `total_sqft` records the total estimate area used for pricing.
- Front and back values may be nullable for historical estimates or estimates where the split is unknown.
- The saved Client Files attachment metadata may include `frontSqft`, `backSqft`, and the related estimate UUID for future search and integration.

### Rationale

Keeping the front and back areas separately supports clearer client communication, later estimate review, reporting, and future scheduling or service-planning features without parsing the generated text artifact.
